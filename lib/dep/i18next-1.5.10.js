...
(function() {
    ...
    function _translate(key, options){
            ...
            if (value !== undefined) {
                if (typeof value === 'string') {
                    ...
                    if (!o.returnObjectTrees && !options.returnObjectTrees) {
                        value = 'key \'' + ns + o.nsseparator + key + ' (' + l + ')\' ' +
                                'returned a object instead of string.';
                        f.log(value);
                    } else {
                        for (var m in value) {
                            // apply translation on childs
                            value[m] = _translate(ns + o.nsseparator + key + '.' + m, options);
                        }
                    }
                }
                found = value;
        ...
        if (found === undefined && o.sendMissing) {
            if (options.lng) {
                sync.postMissing(options.lng, ns, key, o.keyseparator, notfound, lngs);
            } else {
                sync.postMissing(o.lng, ns, key, o.keyseparator, notfound, lngs);
            }
        }
