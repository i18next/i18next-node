(function() {
    ...
    var _isSupported = function(locale) {
        return wrapper.pluralExtensions.rules[locale.split(/[\-_]/)[0]] != false;
    };
    ...
    var _isValidLocale = function(locale) {
        ...
        if (opts.supportedLngs.indexOf) return opts.supportedLngs.indexOf(locale) > -1;

        // any i18next-supported language
        if (opts.supportedLngs === false) return _isSupported(locale);
    ...
    var detectLanguage = function(req, res) {
        ...
        if (typeof req === 'object') {
            ...
            if (!locale) { 
                ...
                var _lng = querystring.query[opts.detectLngQS];
            ...
            if (locale) {
                ...
                locales.some(function(lng) {
                    valid = _isValidLocale(lng);
                    if (valid) {
                        locale = lng;
                    }
                    return valid;
